...
import { useHomeReports } from "../features/incidents/hooks/useHomeReports";
...
export default function Home() {
  ...
  const { user } = useAuth();
  const { reports, loading, error } = useHomeReports();
    ...
    <div className="h-full overflow-y-auto bg-[#f7faf6] pb-16 text-[#102218] md:h-[calc(100vh-6rem)]">
      <section className="relative min-h-95 overflow-hidden bg-[#143624] md:min-h-110">
        ...
        <div className="relative z-10 mx-auto grid min-h-95 max-w-7xl items-center gap-8 px-4 py-10 md:min-h-110 md:grid-cols-[1fr_360px] md:px-8">
          ...
          <div className="rounded-3xl border border-white/30 bg-white/92 p-5 shadow-2xl backdrop-blur">
        ...
        <div className="mb-5 flex items-end justify-between gap-4">
          ...
          <div className="hidden gap-2 sm:flex">
            <button
              ...
              className="grid h-11 w-11 place-items-center cursor-pointer rounded-full border border-[#143624]/15 bg-white text-[#143624] shadow-sm transition hover:bg-[#e7f6ef]"
            >
              <ArrowLeftIcon />
            </button>
            <button
              ...
              onClick={() => scrollCarousel(1)}
              className="grid h-11 w-11 cursor-pointer place-items-center rounded-full border border-[#143624]/15 bg-white text-[#143624] shadow-sm transition hover:bg-[#e7f6ef]"
            >
              <ArrowRightIcon />
            </button>
          </div>
        ...
        >
          {loading
            ? Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="h-65 min-w-57.5 animate-pulse rounded-2xl bg-[#dce9df]"
              />
            ))
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 py-9 md:grid-cols-[1fr_340px] md:px-8">
          <div>
            ...
            <button
              type="button"
              onClick={() => handleReportAction("encontrado")}
              className="mt-6 cursor-pointer rounded-lg bg-[#082c4f] px-5 py-3 text-sm font-black text-white shadow-lg transition hover:bg-[#061f38]"
            >
...
function ActionCard({ title, text, tone, onClick }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={[
        "flex cursor-pointer min-h-40 flex-col items-center justify-center gap-3 rounded-xl p-4 text-center shadow-sm transition",
...
function ReportPreview({ report }) {
  ...
  return (
    <article className="min-w-57.5 snap-start overflow-hidden rounded-2xl border border-[#143624]/10 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      <div className="relative h-36 bg-[#dce9df]">
        {report.photo ? (
          <img src={report.photo} alt={report.speciesLabel} className="h-full w-full object-cover" />
        ) : (
